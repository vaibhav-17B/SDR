          ...
          <DialogTitle className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              {(lead.personal_information?.primary_professional_email || lead.contact_information?.primary_email) && (
                <Button onClick={handleSelectEmails} size="sm">
          ...
          <div className="flex items-center space-x-4 p-4 bg-gradient-to-r from-primary/10 to-primary/5 rounded-lg">
            <div className="flex-shrink-0">
              {lead.personal_information?.picture_url ? (
                ...
                <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center border-4 border-background shadow-lg">
                  <User className="w-10 h-10 text-muted-foreground" />
                </div>
              )}
            </div>
            <div className="flex-grow">
              <h2 className="text-2xl font-bold text-foreground">{lead.personal_information?.full_name || 'Unknown Name'}</h2>
              <p className="text-lg text-muted-foreground">{lead.current_position?.title || 'No position'}</p>
              <p className="text-muted-foreground">{lead.work_experience?.[0]?.company_name || 'No company'}</p>
              <div className="flex items-center space-x-4 mt-2">
                <div className="flex items-center space-x-1">
                  <Mail className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">{lead.personal_information?.primary_professional_email || lead.contact_information?.primary_email || 'No email'}</span>
                </div>
        ...
        <div className="bg-card p-4 rounded-lg border">
          ...
          {lead.personal_information.professional_summary && (
            <div className="md:col-span-2">
              ...
              <p className="ml-2 text-foreground mt-1 italic">
                {lead.personal_information.professional_summary}
              </p>
            </div>
          )}
            ...
            <div className="bg-card p-4 rounded-lg border">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <span className="ml-2 text-foreground">{lead.current_position.title}</span>
                </div>
                <div>
                  <span className="font-medium text-muted-foreground">Department:</span>
                  <span className="ml-2 text-foreground">{lead.current_position.department}</span>
                </div>
                <div>
                  <span className="font-medium text-muted-foreground">Management Level:</span>
                  <span className="ml-2 text-foreground">{lead.current_position.management_level}</span>
                </div>
                <div>
                  ...
                  <span className="ml-2 text-foreground">
                    ...
                    {lead.current_position.total_experience_remaining_months && (
                      <span className="text-muted-foreground"> {lead.current_position.total_experience_remaining_months} months</span>
                    )}
                  </span>
                </div>
                {lead.current_position.description && (
                  <div className="md:col-span-2">
                    ...
                    <p className="ml-2 text-foreground mt-1 italic">{lead.current_position.description}</p>
                  </div>
                )}
              </div>
            </div>
